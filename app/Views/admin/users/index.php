...
<div class="md:pl-64 flex flex-col min-h-screen transition-all duration-300">
    <header class="h-16 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-4 md:px-8 sticky top-0 z-40">
        ...
        <a href="<?= url('/admin/users/create') ?>" class="bg-black text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-gray-800 transition-colors">
    ...
    <main class="p-8 flex-1">
        ...
        <form class="mb-6 flex gap-3" method="GET">
            <div class="relative flex-1 max-w-sm">
                ...
                <input type="text" name="q" value="<?= e($search ?? '') ?>" placeholder="搜索用户 (用户名/邮箱)..." class="w-full bg-white border border-gray-100 rounded-xl py-2 pl-10 pr-4 text-sm focus:ring-black focus:border-black outline-none transition-all shadow-sm">
            </div>
            ...
            <table class="w-full text-sm text-left">
                <thead class="text-xs text-gray-400 uppercase bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-4 font-bold">用户名 Username</th>
                        <th class="px-6 py-4 font-bold">角色 Role</th>
                        <th class="px-6 py-4 font-bold">邮箱 Email</th>
                        <?php \Core\Hook::listen('admin_users_list_header'); ?>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 font-bold text-gray-800">
                             <div class="flex items-center gap-3">
                                <?php if(!empty($u['avatar'])): ?>
                                    <img src="<?= e($u['avatar']) ?>" class="w-8 h-8 rounded-full border border-gray-100">
                                <?php else: ?>
                        ...
                        <td class="px-6 py-4 text-gray-500 font-mono text-xs"><?= e($u['email']) ?></td>
                        <?php \Core\Hook::listen('admin_users_list_column', $u); ?>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-3">
                                <a href="<?= url('/admin/users/edit/' . $u['id']) ?>" class="text-gray-400 hover:text-blue-600 transition-colors">
                                    <i class="fas fa-pen"></i>
                                </a>
                                <?php if($u['id'] != $_SESSION['user_id']): ?>
                                <button onclick="postAction('<?= url('/admin/users/delete/' . $u['id']) ?>', '确定删除此用户？')" class="text-gray-400 hover:text-red-600 transition-colors">
